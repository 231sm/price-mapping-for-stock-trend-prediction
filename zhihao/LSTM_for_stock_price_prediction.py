...
#Adjclose作为预测label，其余作为特征，使用LSTM预测股票价格
...
def get_train_data(batch_size=60,time_step=20,train_begin=0,train_end=int(len(data) * 0.8)):
...
def get_test_data(time_step=20,test_begin=int(1590)):

    data_test=data[test_begin:]

    mean=np.mean(data_test,axis=0)  #获取均值和方差，用于测试时计算预测的Adjclose
...
def train_lstm(batch_size=60,time_step=20,train_begin=0,train_end=int(len(data) * 0.8)):
    ...
    loss=tf.reduce_mean(tf.square(tf.reshape(pred,[-1])-tf.reshape(Y, [-1])))   #使用均方差作为loss函数
...
def prediction(time_step=20):
    ...
    with tf.Session() as sess:
        ...
        print("The accuracy of this predict:",1 - acc)  #准确率
